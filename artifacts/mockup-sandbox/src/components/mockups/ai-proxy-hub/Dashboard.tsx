        ...
        <main>
          ...
          <section id="модели" className="border-b border-[#dededb] py-20 sm:py-28">
            <div className="flex flex-wrap items-end justify-between gap-6">
              <div>
                <div className="mb-4 flex items-center gap-2 text-[11px] font-medium text-[#767676]"><Network size={14} className="text-[#73aabd]" /> Живой каталог</div>
            ...
            <div className="mt-9 flex flex-wrap gap-2">
              {["Все 186", "Текст 70", "Изображения 40", "Видео 61", "Аудио 13"].map((filter, index) => (
                <button key={filter} className={`rounded-full px-4 py-2 text-[11px] font-semibold transition ${index === 0 ? "bg-[#292929] text-white" : "bg-[#edf4f5] text-[#666] hover:bg-[#e2eef1] hover:text-[#466f82]"}`}>{filter}</button>
              ))}
            </div>
            <div className="mt-5 overflow-hidden rounded-[24px] border border-[#dededb] bg-[#f1f1ee]">
              ...
              {[
                ...
                <div key={model} className="grid gap-3 border-b border-[#dededb] px-5 py-4 last:border-0 sm:grid-cols-[1.3fr_.65fr_.65fr_.5fr] sm:items-center sm:gap-4 sm:px-6">
                  <div className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-xl bg-[#e5f3f5] text-[11px] font-bold text-[#5f8f9c]">{mark}</span><div><div className="text-[13px] font-bold">{model}</div><div className="text-[10px] text-[#858580]">{provider} · pay as you go</div></div></div>
                  <div className="text-[12px] text-[#70706c]"><span className="mr-2 text-[10px] text-[#9a9a95] sm:hidden">Контекст</span>{context}</div>
                  <div className="text-[12px] font-semibold text-[#4d7f92]"><span className="mr-2 text-[10px] font-normal text-[#9a9a95] sm:hidden">Вход</span>{input}</div>
                  <div className="text-[12px] font-semibold text-[#4d7f92]"><span className="mr-2 text-[10px] font-normal text-[#9a9a95] sm:hidden">Выход</span>{output}</div>
                </div>
              ))}
            </div>
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 text-[11px] text-[#777]"><span><Check size={13} className="mr-1 inline text-[#72aeb8]" /> Цены обновляются из живого каталога</span><a href="#тарифы" className="font-semibold text-[#4d7f92] underline underline-offset-4">Смотреть все модели <ArrowRight className="ml-1 inline" size={12}/></a></div>
          </section>
                ...
                {[
                  ...
                  { category: "studio", name: "ElevenLabs", provider: "ElevenLabs", context: "—", input: "—", output: "от 1 ₽", note: "озвучка и аудио", mark: "E" },
                   ...
                   <button key={model.name} onClick={() => setActivePrice(index)} className={`group rounded-[24px] border p-5 text-left transition hover:-translate-y-1 hover:border-[#b6dce3] hover:bg-[#edf4f5] ${activePrice === index ? "border-[#9dcbd3] bg-[#e6f3f5]" : "border-[#dededb] bg-[#f1f1ee]"}`}>
                     <div className="flex items-start justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#dceff2] text-[11px] font-bold text-[#5f8f9c]">{model.mark}</span><span className="rounded-full bg-[#dceff2] px-2 py-1 text-[9px] font-semibold text-[#648e9c]">pay as you go</span></div>
